...
def chat_message_html(role, content, sources=None):
    ...
    html = f'<div style="display: flex; flex-direction: column; width: 100%; align-items: {"flex-end" if is_user else "flex-start"};">' \
           f'<div class="role-label" style="text-align: {"right" if is_user else "left"};">{icon if not is_user else ""} {label_text} {icon if is_user else ""}</div>' \
           f'<div class="bubble {bubble_class}">{content}{source_html}</div>' \
           f'</div>'
